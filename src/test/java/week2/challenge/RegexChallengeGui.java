...
public class RegexChallengeGui implements ItemListener {
    ...
    private void addComponentToPane(Container pane) {
        ...
        for (int i = 0; i < RegexChallengeTask.TASKS.length; i++) {
            ...
            TextNote text = new TextNote(regexChallengeTask.description());
            ...
            input.addActionListener(e -> {
                ...
                output.setText(check(regexChallengeTask.automaton(), text1));
            });
            card.add(text);
            card.add(input);
            cards.add(card, regexChallengeTask.name());
        }
